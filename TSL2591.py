...
def sensor_read():
    values = tsl.get_current()
    brightness = values['full']
    return brightness

sec_at_start = time_in_seconds(time.ctime())
time.sleep(0.5)
tsl2591_start_time = time.ctime()
print('tsl2591.py started at: ',\
      tsl2591_start_time, 'equals:', sec_at_start)
logging.info('tsl2591.py started ')

if sec_at_start >= 71940:
    time.sleep(1)
    spot_gallery.on()
    print('direkt start, gallery_on')
    logging.info('direkt start, gallery_on')
